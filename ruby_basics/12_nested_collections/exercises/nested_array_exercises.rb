...
def find_favorite(array_of_hash_objects)
  ...
  # 
  # 1 iterate through array, then iterate through hash, 
  # if is_my_favorite? => true then return hash
  # if is_my_favorite? => false then return hash
  
  array_of_hash_objects.each do |result|
      if result[:is_my_favorite?] == true
        return result
      end
  end
  nil
